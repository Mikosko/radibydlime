...
The homepage, `/kapitoly/` index, and `/projekty/<slug>/` route query the same published Project records. The [Project contract](../src/content/projects/SPEC.md) defines stable identity, draft/archive visibility, metadata, and validation. The [site layout contract](../src/layouts/site/SPEC.md) defines the shared shell. The current sample uses locally authored review photography and Markdoc narrative; published media-catalog heroes and local SVG images remain covered by build fixtures. Two [article blocks](../src/blocks/BLOCKS.md) support full-width catalog images and prose paired with a catalog image or local decoration. An optional [ArticleGallery](../src/components/article-gallery/SPEC.md) presents selected images after a Project's narrative as a static strip with a small native-dialog enhancement for full-size viewing. No JournalEntry collection is implemented.

Owner-authored sale listings and static item details use a SaleItem collection with email enquiries; no checkout or commerce backend is introduced. Workshops now have owner-authored static listings and detail pages with email enquiries and manually maintained capacity. Do not pre-build booking backends, before/after blocks, editors, authentication, commerce, or an extensive component library. Do not create implementation directories for hypothetical features.
