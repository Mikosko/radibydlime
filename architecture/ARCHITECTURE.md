...
The homepage and `/projekty/<slug>/` route query the same published Project records. The [Project contract](../src/content/projects/SPEC.md) defines stable identity, draft/archive visibility, metadata, and validation. The [site layout contract](../src/layouts/site/SPEC.md) defines the shared shell. The current sample uses locally authored review photography and ordinary Markdoc narrative; published media-catalog heroes and local SVG images remain covered by build fixtures. No JournalEntry or custom content blocks were needed to validate this slice.
