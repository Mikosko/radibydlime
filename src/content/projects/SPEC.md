...
Markdoc supplies ordinary narrative elements. Raw HTML is disabled; unsupported tags fail the build. The Project route derives its desktop article-orientation rail from rendered level-two Markdoc headings and their generated fragment IDs, so authors do not duplicate a table of contents in frontmatter. The rail is omitted where the layout no longer has a left sidebar at narrower widths. The hero media reference is the first implemented cross-content relationship. No custom block tags or generalized queries are implemented. Future relationship fields also need canonical-ID target validation. Narrative headings start at level two because the route owns the page's level-one title.
